...
fun main()
{
    ...
    val firstUnique = FirstUnique(arr)
...
internal class FirstUnique(nums: IntArray) {
    private var unique: MutableSet<Int> = LinkedHashSet()
    var all: MutableSet<Int> = HashSet()
    fun showFirstUnique(): Int {
        return if (unique.isEmpty()) -1 else unique.iterator().next()
    }

    fun add(value: Int) {
        if (all.add(value)) unique.add(value) else unique.remove(value)
    }

    init {
        for (element in nums) add(element)
    }
}